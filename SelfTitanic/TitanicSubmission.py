...
def cleaner(arr):
    ...
    arr["Age"] = age
    if("Survived" not in arr):
        return arr
    y_train = arr["Survived"]
    arr = arr.drop(["Survived"], inplace=False, axis=1)
    return arr, y_train
...
if(load):
    ...
    hi = open("titanicplot.txt", "w")
    hi.close()

with open('titanicplot.txt', mode='r', newline='') as file:    
    ...
    ax.plot(X,Y)
    mpl.pyplot.savefig("titanicplot.png")
...
Pid = TestData["PassengerId"].tolist()
...
testpred = testpred.astype(np.float64)
...
"""with open("Submission.csv", "w") as subfil:
    subfil.writelines("PassengerId,Survived"+"\n")
    for z in range(len(testpred)):
        if(testpred[z][0] == np.nan):
            testpred[z][0] = 0
        subfil.writelines(str(Pid[z])+","+str(round(testpred[z][0]))+"\n")"""
